...
export const useAddItemsMutation = (tableId) => {
    ...
    return useMutation({
        mutationFn: async ({ orderId, items }) => {
            ...
            if (finalOrderId) {
                await Promise.all(items.map((item) => addOrderItem({
                    id: item.dish_id,
                    order: finalOrderId,
                    dish: item.dish_id,
        ...
        onMutate: async ({ orderId, items }) => {
            ...
            queryClient.setQueryData(['order', tableId], (old) => {
...
export const useCancelItemMutation = (tableId) => {
    const queryClient = useQueryClient();
    return useMutation({
...
export const useSplitMergeMutation = (currentTableId) => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async ({ mode, targetTableId, items }) => {
            if (mode === 'merge') {
        ...
        onSettled: (_, __, variables) => {
            if (currentTableId)
                queryClient.invalidateQueries({ queryKey: ['order', currentTableId] });
            if (variables.targetTableId)
                queryClient.invalidateQueries({ queryKey: ['order', variables.targetTableId] });
            queryClient.invalidateQueries({ queryKey: ['tables'] });
        }
    });
};
